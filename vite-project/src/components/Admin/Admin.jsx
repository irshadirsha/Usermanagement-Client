...
function Admin() {
    ...
    const deleteUser=(userId)=>{
        axios.post('http://localhost:3000/admin/deleteuser',{userId},{withCredentials: true }).then((res)=>{
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const { data } = await axios.post('http://localhost:3000/admin/auth', { token }, { withCredentials: true });
  ...
  return (
    <div>
          ...
          <button onClick={() => navigate('/add-user')} style={{ position: 'relative', left: 20 }}>Add User</button>
            <button onClick={adminLogout} style={{ position: 'relative', left: 40 }}>Logout</button>
...

                    {
                        users.filter((user) =>
                            user.username.toLowerCase().includes(query)).map((user, index) => {
                                return (
                                    <tr key={index}>
                                        <td>{user.image ? <img src={`/Images/${user.image}`} alt="placehold"height={130} width={100} /> :<img src="https://static-00.iconduck.com/assets.00/profile-minor-icon-256x256-6u3v5w0z.png" alt="placehold" width={30} />}</td>

                                        <td style={{color:'blueviolet'}}>{user.username}</td>
